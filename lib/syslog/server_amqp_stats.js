...
function updateSystem() {
	...
	console.log(arg.disk);
	console.log(arg.cpu_avg1);
	});
}
...
function update5minStatistics() {
   updateTimeStatistics(Math.floor(timestamp/ 100 - (timestamp/100) %5), "stat5min")
   updateTimeStatistics(Math.floor(timestamp/ 10000), "statHour")
   updateTimeStatistics(Math.floor(timestamp/ 1000000), "statDay")
   updateTimeStatistics(Math.floor(timestamp/ 100000000), "statYear")
...
function updateTimeStatistics(datest, bucket) {
   
   db.get(bucket, datest, function(err, stat, meta) {
      if (err) {
         db.save(bucket, datest, { count: sub_array(packetsReceived, last5min)},
	    {headers:{ "X-Riak-index-date_int": datest }}
	  );
      } else {
          db.save(bucket, datest, { count: add_array(stat.count, sub_array(packetsReceived, last5min)) },
	    {headers:{ "X-Riak-index-date_int": datest }}
	  );
    }
  });
