...
function updateTotalStatistics() {
	...
	if (err) {
	    ...
	    var stats = {packetsReceived: packetsReceived[0] + packetsReceived[1] + packetsReceived[2] + packetsReceived[3], lastupdate: timestamp };
	    db.save('statistics', totalKey, stats);
	} else {
	    packs.packetsReceived = packs.packetsReceived +
                                             (packetsReceived[0] - last[0]);
                                             + (packetsReceived[1] - last[1]);
                                             + (packetsReceived[2] - last[2]);
                                             + (packetsReceived[3] - last[3]);
