...
var diskUsage = [0,0];
var minDiskSpace = 100; // minimum Req. space on node
...
function updateSystem() {
	...
	db.get('system', member, function(err, pack, meta) {
		...
		db.save('system', meta.key, JSON.stringify(data), meta);
		}); 
	});
}
...
//		});
   ...
   if (diskUsage[1] > minDiskSpace || diskUsage[0] == 0) { // or init
	forceFlush = false;
	   q.shift();
   ...
   if (messages.length > 1 || forceFlush) {
	forceFlush = false;
        var o = messages;
        messages = []; // race?
