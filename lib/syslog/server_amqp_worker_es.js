...
// connect to multiple-endopoints (HA/load balancing. Only masters will be sending data)
zmqconfig.server.forEach(function(arg) { sock.connect(arg); });
...
function setup() {
    ...
    setInterval(updateAliases, 3200000);
    //setInterval(optimizeIndexes, 36000000); // every hour
...
function optimizeIndexes() {
    console.log("Optimizing " + get_previous_index());
    cli.optimize(get_previous_index(), {"max_num_segments":1});
}

function updateAliases() {
console.log("updating aliases " + get_alias() + " -> " + get_index());
    cli.createIndex(get_index(), {"actions":[{"add":{"index":
                                                     get_index(),
                                                     "alias": get_alias()}}]})
        .exec();
}
function storeMessages() {
    var locm = messages;
    messages = []; //
    // make sure we have aliases in place
    cli.bulk(locm, {})
...
function storeMessage(m) {
    ...
    try {
        syslogParser.parse(rawMessage, function(parsedMessage) {
            ...
            shasum.update(parsedMessage.originalMessage);
            parsedMessage.cryptosignature = shasum.digest('hex');
...
// do some cleanup
            ...
            messages.push({"index":{"_index":get_index_date(parsedMessage.time), "_type":get_index_type()}});
