...
def getindexaliasdateday(dt):
    index_name = 'searchindex-%.4i%.2i%.2i'% (dt.year, dt.month, dt.day)
    return index_name
...
start=datetime.datetime.now() - timedelta(int(sys.argv[2]) )
#start=datetime.datetime.now() - timedelta(4 )
...
for f in daterange(start, end):
	for h in range(0, 24):
		...
		try:
			...
			conn.indices.add_alias(getindexaliasdateday(d2), indices=[ getdateindex(d2)])
			print "%s -> %s" %(getindexaliasdateday(d2),  getdateindex(d2))
		except Exception, e:
			print e
